...
const createNewRawBlock = data => {
  ...
  const newDifficulty = findDifficulty(getBlockchain());
...
const calculateNewDifficulty = (newestBlock, blockchain) => {
  ...
  if (timeTaken < timeExpeted / 2) {
    return lastCalculatedBlock.difficulty + 1;
  } else if (timeTaken > timeExpeted * 2) {
    return lastCalculatedBlock.difficulty - 1;
  } else {
...
const hashToHashMathcesDifficulty = (hash, difficulty) => {
  const requiredZeros = "0".repeat(difficulty);
  console.log("Trying difficulty: ", difficulty, "with hash", hash);
